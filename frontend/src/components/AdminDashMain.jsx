...
const AdminDashMain = ({ type }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`https://hospital-appointment-booking-system.onrender.com/admin/${type}`);
    ...
    const handleDelete = async (id, delType) => {
        try {
            const res = await fetch(`https://hospital-appointment-booking-system.onrender.com/admin/${delType==="doctor" ? "Doctor_delete" : "Patient_delete"}/${id}`, {
                method: 'DELETE',
            })
